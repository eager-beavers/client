...
const UserDialog = (props: prop) => {
    ...
    const handleOk = () => {
        ...
        if (props.userModel === undefined) {
            ...
            const copyUserList = cloneDeep(userList);

            for (let i = 0; i < copyUserList.length; i += 1) {
                if (copyUserList[i].getUserUniqueNum() === props.userUniqueNum) {
                    copyUserList[i].setUserName(name);
                    copyUserList[i].setMobileNumber(phoneNumber);
                    copyUserList[i].setUserPw(userPw);
                }
            }
    ...
    return (
        <>
            ...
            <Modal className="modalStyle" bodyStyle={{backgroundColor: 'papayawhip'}} title="회원 등록"
                   visible={isModalVisible} onCancel={closeModal} onOk={handleOk}>
                <div className="flex flex-col">
                    이름 입력: <input
                    className="border border-solid rounded-md focus:outline-none focus:border-amber-600 border-amber-400"
                    type="text" value={name} onChange={onChangeName}/>
                    핸드폰 번호 입력: <input
                    className="border border-solid rounded-md focus:outline-none focus:border-amber-600 border-amber-400"
                    type="text" value={phoneNumber} onChange={onChangePhoneNumber}/>
                    비밀번호 입력: <input
                    className="border border-solid rounded-md focus:outline-none focus:border-amber-600 border-amber-400"
                    type="password" value={userPw} onChange={onChangeUserPw}/>
                </div>
            </Modal>
        </>
    );
